      ...
      def request(request)
        ...
        while tries < request.tries
          ...
          unless [429, 500].include?(response.code)
            raise ShopifyAPI::Errors::HttpResponseError.new(code: response.code.to_i, errors: response.body["errors"]),
              error_message
          end

          if tries == request.tries
            raise ShopifyAPI::Errors::HttpResponseError.new(code: response.code, errors: response.body["errors"]),
              error_message if request.tries == 1
